...
holdinggreen = 0
holdingred = 0
df = pd.read_csv("Nasdaq 100 Constituents - Nasdaq Component Daily Changes 18-19.csv")
pastclose = 0
currentclose = 1
futureclose = 2
portfolioovertime = []
...
while futureclose != 251:
    ...
    print(dflistoldholding)
    print(dflistnewholding)
    ...
    investmentoriginal = investments
    holdingiterator = 0
    portfoliolist = []
    ...
    while holdingiterator != stocksholding:
        ...
        if averagefinalchangedranked > 0:
            investmentchange1 = finalchangeholdingranked[holdingiterator] * investments - investments
            investmentchange1 = abs(investmentchange1)
            investmentchange = investmentchange1
            portfoliolist.append(investmentchange)
            holdingiterator = holdingiterator + 1
            print(portfoliolist)
            print("Investment change is " + str(investmentchange))

            if investmentchange > investmentoriginal:
                holdinggreen = holdinggreen + 1
            else:
                holdingred = holdingred + 1
        elif averagefinalchangedranked < 0:
